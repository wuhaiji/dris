...
- **不做宏展开**：扫描的是源码 AST（`syn`），宏生成的组件/构造函数可能看不到。
- **不做类型检查**：只做“结构化扫描 + 生成”，很多错误会在生成阶段或编译阶段暴露。
- **会解析 `src/` 下的所有 `.rs` 文件**：不依赖 `mod` 树，也不走 `cfg` 条件裁剪；因此即使某个文件当前未被引用/未启用 feature，它也必须能被 `syn` 正常解析。
- **不要用 `cfg` 做“互斥组件定义”**：例如按平台写两份同名 `#[component] struct Logger`，Rust 编译时只会选中一份，但生成器会同时看到两份并报“组件类型重复”。建议把 `cfg` 放在组件内部实现细节里，不要切换组件类型/构造函数本身。
- **构造函数限制**：`#[constructor]` 必须是 `pub fn`，不支持泛型，不允许 `self` 参数。
- **不支持 `&mut` 注入**：请用内部可变性（`RefCell`/`Mutex`/`RwLock` 等）。
- **Prototype + `&T` 的一个重要约束**：生成器会为 Prototype 依赖创建临时值再传引用；因此请不要在返回的对象里长期持有这个引用（否则会产生悬垂引用）。如果需要长期持有依赖，请改用 `Arc<T>`/`Rc<T>` 或把当前组件也变成单例并通过 getter 借用。
